...
When submitting jobs for your exercises, you should use the \code{comchem} queue, preferably.
...
of the terminals.
When submitting more intensive and lengthy jobs, you should use the \code{coms} queue. This queue is better suited also for more memory demanding
...
Again, long calculations should run on \code{coms}, short ones (e.g. tests, exercises) on \code{comchem}.
...
As previosuly mentioned, \code{vi} is a great multipurpose text editor. You can find a cheat sheet with all its main commands at the end of this guide.
...
This command will start the program \code{nedit} and open the file \code{filename} for editing. If such file does not exist, it will ask to create a new one.
Menu items in \code{nedit} are basically self-explanatory and allow actions common to most text editors
